...
import { Calendar, Wallet, TrendingUp, MapPin, Clock, ArrowRight, Plane, Navigation } from 'lucide-react';
...
export default function TripDashboard({ plan }: TripDashboardProps) {
  ...
  const stats = [
    ...
    { icon: <TrendingUp className="w-5 h-5" />, label: 'Daily Average', value: `${curr}${avgDaily.toLocaleString()}`, sub: 'per day activities' },
            ...
            {plan.flights.map((flight) => (
              <div key={flight.id} className="glass rounded-xl p-4 flex items-center justify-between">
                ...
                <div className="text-right">
                  <p className="font-display font-bold text-foreground">{curr}{flight.price.toLocaleString()}</p>
                  {showDual && flight.priceHome && (
                    <p className="text-xs text-muted-foreground">≈ {homeCurr}{flight.priceHome.toLocaleString()}</p>
                  )}
                </div>
              </div>
            ))}
          ...
          <div className="grid gap-3">
            {plan.hotels.map((hotel) => (
              <div key={hotel.id} className="glass rounded-xl p-4 flex items-center justify-between">
                ...
                <div className="text-right">
                  <p className="font-display font-bold text-foreground">{curr}{hotel.pricePerNight.toLocaleString()}</p>
                  {showDual && hotel.pricePerNightHome && (
                    <p className="text-xs text-muted-foreground">≈ {homeCurr}{hotel.pricePerNightHome.toLocaleString()}</p>
                  )}
