...
serve(async (req) => {
  ...
  try {
    const { origin, destination, days, travelers, travelerBreakdown, userBudget, currency, homeCurrency, homeCurrencyCode, destCurrencySymbol, tripType, pace, startDate } = await req.json();
    ...
    const prompt = `Create a ${days}-day trip from ${origin} to ${destination} for ${travelers} travelers (${travelerBreakdown?.male || 0} male, ${travelerBreakdown?.female || 0} female, ${travelerBreakdown?.kids || 0} kids). Budget: ${homeCurrency}${userBudget}. Start: ${startDate}. Trip type: ${tripType}. Currency: ${currency}, home: ${homeCurrency}(${homeCurrencyCode}).
